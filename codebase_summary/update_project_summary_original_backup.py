...
class EnhancedProjectSummaryGenerator:
    ...
    def generate_summary(self, enable_fix_components=False):
        ...
        try:
            ...
            # Function statistics are already calculated during analysis
            # The totals are maintained in the analysis object regardless of mode
    ...
    def _analyze_code_files_enhanced(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "missing_breadcrumbs": [],
            "file_analysis": [],  # Will be populated based on verbosity
            "language_breakdown": {},
            "complexity_metrics": {},
            "documentation_gaps": [],  # Top files with missing docs
            ...
            for file in files:
                if any(file.endswith(ext) for ext in code_extensions):
                    ...
                    if file_info:
                        ...
                        analysis["total_lines_of_code"] += file_info.get("lines_of_code", 0)
                        ...
                        # Track documentation gaps (top files with missing docs)
                        missing_count = len(file_info.get("missing_breadcrumbs", []))
                        if missing_count > 5:  # Only track files with significant gaps
                            analysis["documentation_gaps"].append({
                                "file": rel_path,
                                "undocumented": missing_count
                            })

                        # Store detailed file info only if not in minimal mode
    ...
    def _analyze_code_file_enhanced(self, file_path: str, rel_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "functions": [],  # Removed to reduce token usage
                "missing_breadcrumbs": missing_breadcrumbs,  # Keep full list for missing_breadcrumbs tracking
    ...
    def _create_enhanced_summary(self, config: Dict[str, Any], structure: Dict[str, Any], 
        ...
        return {
            "project_name": config.get("project_name", project_info["name"]),
            "version": version,
            "updated_at": datetime.datetime.now().isoformat() + "Z",
            "description": config.get("project_description", project_info["description"]),
            ...
            # Code analysis with function statistics (optimized based on mode)
            "code_analysis": self._optimize_code_analysis(analysis),
    ...
    def _optimize_code_analysis(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        """Optimize code analysis based on verbosity mode"""
        import copy
        # Work on a copy to preserve original data for other reports
        optimized = copy.deepcopy(analysis)
        
        # Sort documentation gaps by most missing
        if optimized.get("documentation_gaps"):
            optimized["documentation_gaps"].sort(key=lambda x: x["undocumented"], reverse=True)
            # Keep only top 10
            optimized["documentation_gaps"] = optimized["documentation_gaps"][:10]
        
        # Calculate coverage percentage
        total_funcs = optimized.get("total_functions", 0)
        doc_funcs = optimized.get("documented_functions", 0)
        coverage = (doc_funcs / max(1, total_funcs)) * 100 if total_funcs > 0 else 0
        
        # Add summary stats
        optimized["coverage_percentage"] = coverage
        optimized["missing_count"] = total_funcs - doc_funcs
        
        # Always use optimized mode unless explicitly verbose
        if "--verbose" not in sys.argv:
            # Remove detailed file analysis
            optimized.pop("file_analysis", None)
            # Keep only summary of missing breadcrumbs
            if optimized.get("missing_breadcrumbs"):
                # Group by directory
                dir_counts = {}
                for item in optimized["missing_breadcrumbs"]:
                    dir_path = os.path.dirname(item["file"])
                    if dir_path not in dir_counts:
                        dir_counts[dir_path] = 0
                    dir_counts[dir_path] += len(item["missing"])
                
                optimized["missing_breadcrumbs_summary"] = {
                    "total_missing": sum(dir_counts.values()),
                    "by_directory": dict(sorted(dir_counts.items(), key=lambda x: x[1], reverse=True)[:10])
                }
                # Remove detailed list
                optimized.pop("missing_breadcrumbs", None)
    ...
    def _detect_project_info(self) -> Dict[str, str]:
        ...
        # Priority 1: package.json (Node.js projects)
        ...
        # Priority 2: setup.py (Python projects)
        setup_py = self.project_root / "setup.py"
        if setup_py.exists():
            try:
                with open(setup_py, 'r', encoding='utf-8') as f:
                    content = f.read()
                    # Extract name from setup()
        ...
        # Priority 3: pyproject.toml (Modern Python)
        ...
        # Priority 4: Cargo.toml (Rust projects)
        ...
        # Priority 5: README.md first paragraph
                        ...
                        if lines:
                            ...
                            for line in lines[1:]:
                                if (len(line) > 20 and 
                                    ...
                                    clean_desc = re.sub(r'\*\*([^*]+)\*\*', r'\1', line)  # Remove bold
                                    clean_desc = re.sub(r'\*([^*]+)\*', r'\1', clean_desc)  # Remove italic
                                    clean_desc = re.sub(r'`([^`]+)`', r'\1', clean_desc)  # Remove code
        ...
        # Priority 6: Directory name fallback
    ...
    def _generate_enhanced_reports(self, analysis: Dict[str, Any], 
        ...
        """Generate enhanced reports"""
        # Use pre-calculated totals or calculate from file analysis
        total_functions = analysis.get("total_functions", 0)
        documented_functions = analysis.get("documented_functions", 0)
        language_breakdown = analysis.get("language_breakdown", {})
        missing_list = analysis.get("missing_breadcrumbs", [])
        
        # If totals not available, calculate from file analysis
        if total_functions == 0 and analysis.get("file_analysis"):
        ...
        missing_count = analysis.get("missing_count", sum(len(item.get("missing", [])) for item in missing_list))
        coverage_percentage = analysis.get("coverage_percentage", (documented_functions / max(1, total_functions)) * 100)
...
"""
    ...
    def _save_summary(self, summary: Dict[str, Any]):
        """Save enhanced summary to file with directory regeneration"""
        try:
            # Ensure parent directory exists
            os.makedirs(os.path.dirname(self.summary_path) if os.path.dirname(self.summary_path) else ".", exist_ok=True)
            
            # Add generator information
            summary_with_generator = {
            ...
            with open(self.summary_path, 'w', encoding='utf-8') as f:
                json.dump(summary_with_generator, f, indent=2, ensure_ascii=False)
                
        except Exception as e:
            print(f"⚠️  Failed to save summary to {self.summary_path}: {e}")
            # Try backup location
            backup_path = "codebase_summary_backup.json"
            try:
                with open(backup_path, 'w', encoding='utf-8') as f:
                    json.dump(summary_with_generator, f, indent=2, ensure_ascii=False)
                print(f"✅ Saved summary to backup location: {backup_path}")
            except Exception as backup_error:
                print(f"❌ Failed to save summary to backup location: {backup_error}")
                raise
...
Options:
  ...
  --fix-components         Enable automatic components.json path corrections
  --minimal                Generate minimal summary (reduces token usage by ~80%)
  --version                 Show version information
